...
void *K_means(void *arg) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &(thread_intervals[id_thread].start));
    ...
    clock_gettime(CLOCK_MONOTONIC, &thread_intervals[id_thread].end);

    printf("\nThread(%ld) processou por %f segundos", id_thread,
           ((double) (thread_intervals[id_thread].end.tv_sec - thread_intervals[id_thread].start.tv_sec)) +
           ((double) (thread_intervals[id_thread].end.tv_nsec - thread_intervals[id_thread].start.tv_nsec)) /
           1000000000.0);
...
int main(int argc, char *argv[]) {
    ...
    double totalTimeElapsedProcessing =
            ((double) (thread_intervals[NTH - 1].end.tv_sec - thread_intervals[NTH - 1].start.tv_sec)) +
            ((double) (thread_intervals[0].end.tv_nsec - thread_intervals[0].start.tv_nsec)) /
            1000000000.0;
