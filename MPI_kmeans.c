...
void soma_ponto_no_centroid(int id_centroid, int id_ponto) {

    CENTROIDE *centroide = &CENTROIDES[id_centroid];
    PONTO ponto = PONTOS[id_ponto];
...
int processo_responsavel(int i) {
    return i % SIZE + 1;
}
...
int main(int argc, char *argv[]) {
    ...
    if (RANK == 0) {
        for (int i = 0; i < NUM_PONTOS; i++) {
            int id_ponto, id_centroid;
            int id_processo = processo_responsavel(i);

            printf("Pega resultado do ponto %d do processo %d | SIZE=%d", i, id_processo, SIZE);

            MPI_Recv(&id_ponto, 1, MPI_INT, id_processo, TAG, MPI_COMM_WORLD, &st);
            MPI_Recv(&id_centroid, 1, MPI_INT, id_processo, TAG, MPI_COMM_WORLD, &st);

            soma_ponto_no_centroid(id_centroid, id_ponto);
        }
    } else {
        for (int id_ponto = RANK - 1; id_ponto < NUM_PONTOS; id_ponto += SIZE) {
            PONTO ponto = PONTOS[id_ponto];
            CENTROIDE *centroide;
            int i, distancia_atual, menor_distancia = INT_MAX, indice;
            ...
            MPI_Send(&id_ponto, 1, MPI_INT, 0, TAG, MPI_COMM_WORLD);
            MPI_Send(&indice, 1, MPI_INT, 0, TAG, MPI_COMM_WORLD);
        }
    }
